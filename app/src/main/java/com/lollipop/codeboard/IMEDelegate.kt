...
class IMEDelegate(
    ...
    private fun input(block: (InputConnection) -> Unit) {
    ...
    fun onDecorationTouch(
        ...
        log("onDecorationTouch: key = $key, isPressed = $isPressed, current = ${decorationKey}")
        if (KeyboardConfig.simulationMode) {
            input {
                it.sendKeyEvent(KeyEvent(KeyEvent.ACTION_DOWN, key.keyCode))
            }
        } else {
            if (isPressed) {
                // 处理按下的状态，分为2个情况
